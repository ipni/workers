#!/usr/bin/env bash
# Open an SSH tunnel to a box's k3s API server and print the KUBECONFIG line
...
#
# Usage:  ./scripts/kubectl-tunnel.sh <box> [local-port]
...
WORKING="kubeconfigs/.${HOST}-tunnel.yaml"
...
if (exec 3<>"/dev/tcp/127.0.0.1/${PORT}") 2>/dev/null; then
  ...
  if [[ -f "$WORKING" ]]; then
    echo "If it is the ${HOST} tunnel:  export KUBECONFIG=$(pwd)/${WORKING}" >&2
  fi
  echo "Otherwise pick another port:  $0 ${HOST} <local-port>" >&2
  exit 1
fi
...
if ! HOSTVARS=$(ansible-inventory --host "$HOST" 2>"$INV_ERR" </dev/null); then
...
echo "Opening tunnel to ${HOST} (${IP}) on local port ${PORT}..."
